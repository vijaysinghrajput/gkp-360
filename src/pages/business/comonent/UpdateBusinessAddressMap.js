...
const GOOGLE_MAPS_API_KEY = "AIzaSyCetgdiWmPHMxtMMAwbnQ-ogsMj27EQw";

const UpdateBusinessAddressMap = ({ lat, lng, onAddressSelected }) => {
  const DEFAULT_LOCATION = { lat: lat || 26.7606, lng: lng || 83.3732 }; // Gorakhpur fallback
  ...
  const handlePredictionClick = (description) => {
    ...
    geocoder.geocode({ address: description }, (results, status) => {
      if (status === "OK" && results[0]?.geometry) {
        ...
        alert("Failed to retrieve the location for the selected address.");
      }
    });
  };
  ...
  const fetchAddressDetails = async (latitude, longitude) => {
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        console.warn("No address results found for the given coordinates.");
        alert("No address found for the selected location. Please try again.");
      }
    } catch (error) {
...
export default UpdateBusinessAddressMap;
